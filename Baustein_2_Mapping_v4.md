...
## 2.7 Die Amplituden-Konstante (Lösung von K6)

Die Amplitude \( A \) eines Wellenbeitrags und die Energie \( E \) des
Hexaphotons beschreiben **dieselbe Eigenschaft** — wie kräftig die Welle
ist — in zwei Maßen. Sie sind keine unabhängigen Größen, sondern
ineinander umrechenbar.

Die klassische Wellenoptik gibt den Zusammenhang vor: Die Energie einer
Welle ist proportional zum **Quadrat** der Amplitude. Umgekehrt:

\[
A = \sqrt{\frac{E}{k}}
\]

\( k \) ist eine **feste Umrechnungskonstante**. Sie stellt die Einheiten
richtig (Energie ↔ Amplitude) — vergleichbar dem festen Faktor, der Meter
in Kilometer umrechnet.

> **\( k \) ist eine reine Konstante.** Sie hängt **nicht** ab von:
> der Photonenzahl (das ist die Summe, Abschnitt 2.4); den Primärgrößen
> \( f, P, d \) (da \( E = h f \) die Frequenz bereits enthält, wäre eine
> \( f \)-Abhängigkeit von \( k \) eine Doppelzählung); oder der
> Polarisation (eine Richtung des Schwingens, keine Stärke). \( k \) ist
> die Einzel-Umrechnung für *ein* Hexaphoton, gebildet bevor irgendeine
> Summe oder Wechselwirkung ins Spiel kommt.

Damit ist \( A \) vollständig durch \( E \) bestimmt — konsistent mit den
Constraints C1, C2 aus Baustein 1: das Hexaphoton legt seine Welle
vollständig fest, ohne offene Parameter.

---

## 2.8 Übergang zur Modalbasis (Lösung von K7)

Die Abschnitte 2.2–2.5 beschreiben Licht als **ebene Wellen** — Wellen mit
geraden, parallelen Fronten. Das ist die natürliche Beschreibung für Licht
im freien Raum.

Für die Anbindung an die Mie-Streutheorie (Baustein 4) passt diese
Beschreibung jedoch nicht unmittelbar: Eine Kugel streut Licht in
**kugelförmig nach außen laufende** Wellenfronten. Für solche Probleme ist
eine andere Beschreibung praktischer — die **sphärischen Moden**
(sphärische Harmonische).

**K7 ist die Brücke zwischen beiden Beschreibungen.** Es ist *dieselbe*
Lichtwelle, nur in einem anderen, der Kugelform angepassten
Koordinatensystem ausgedrückt.

> **Analogie.** Eine Stadt lässt sich mit einem Straßenraster (Nord-Süd,
> Ost-West) oder mit Ringen und Speichen um das Zentrum beschreiben.
> Beides beschreibt dieselbe Stadt; für eine kreisrunde Altstadt sind
> Ringe praktischer. K7 ist der „Umrechner" zwischen beiden Karten.

**Wichtig — hier wird nichts Neues erfunden.** Die Übersetzung von ebenen
Wellen in sphärische Moden ist etablierte mathematische Physik, seit über
einem Jahrhundert bekannt (die *ebene-Welle-Entwicklung nach
Kugelfunktionen*; von Mie 1908 verwendet). K7 erfindet diese Mathematik
nicht — es benennt nur, **an welcher Stelle** des Hexaphotone-Systems sie
andockt:

- Das Mapping F erzeugt aus einem Hexafeld eine Welle in
  **ebene-Wellen-Darstellung** (Abschnitt 2.2–2.4).
- Vor der Anwendung eines kugelbezogenen Operators \( T_M \) (Mie) wird
  diese Welle per Standard-Entwicklung in die **sphärische Moden-Darstellung**
  übersetzt.
- Nach der Streuung kann das Ergebnis wieder in die ebene-Wellen-Darstellung
  zurückübersetzt werden.

Die konkrete Formel der Entwicklung ist Standard und wird dort eingesetzt,
wo tatsächlich gerechnet wird — in **Baustein 5** (unabhängige Validierung).
Für die konzeptuelle Vollständigkeit des Mappings F genügt die Feststellung:
Der Wechsel der Darstellung ist eine bekannte, verlustfreie Übersetzung,
keine zusätzliche physikalische Annahme.
...
## 2.10 Status der Klärungspunkte

- **K5 — Polarisationsvektor.** ✓ Gelöst (Abschnitt 2.3, Konvention B).
- **K6 — Amplituden-Konstante \( A \).** ✓ Gelöst (Abschnitt 2.7,
  \( A = \sqrt{E/k} \), \( k \) feste Konstante).
- **K7 — Übergang zur Modalbasis.** ✓ Gelöst (Abschnitt 2.8, konzeptuell;
  konkrete Rechnung in Baustein 5).

Damit sind alle offenen Klärungspunkte des Mappings F geschlossen.

---

## 2.11 Kommentar zur Interpretation
